...
const StarMatch = () => {
    const [stars, setStars] = useState(utils.random(1,2));
    const [availableNums, setAvailableNums] = useState(utils.range(1,2));
    const [candidateNums, setCandidateNums] = useState([]);

    const candidatesAreWrong = utils.sum(candidateNums) > stars;
    const gameIsDone = availableNums.count == 0;
    ...
    const onNumberClick = (number, status) => { 
        ...
        if(utils.sum(newCandidateNums) != stars){
            ...
            const newAvailableNums = availableNums.filter(an => !newCandidateNums.includes(an));
